...
with st.expander("🔍 SQL 및 인사이트 보기"):
    ...
    st.write("- **인사이트 1:** 서초구·용산구·강남구는 의료복지시설 1개당 담당 고령인구가 많아
   공공 의료복지 서비스 접근성이 상대적으로 낮은 지역이다.")
    st.write("- **인사이트 2:** 차트1(재가시설)과 차트2(의료시설) 모두에서 용산구가 
   취약지역으로 나타나 복지 인프라 전반의 확충이 필요하다고 판단됨.")
    st.warning("""
⚠️ 분석 한계: 본 차트는 시설 수만을 기준으로 분석하였으며, 
각 시설의 수용 정원·현원 데이터가 포함되지 않아 
실제 서비스 공급량과 차이가 있을 수 있습니다.
""")
