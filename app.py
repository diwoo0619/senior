...
with st.expander("🔍 SQL 및 인사이트 보기"):
    ...
    st.write("- **인사이트 1:** 고령화율이 가장 높은 자치구가 반드시 가장 많은 재가시설을 보유하고 있지는 않음을 알 수 있습니다.
    노원구를 보면 고령화율은 중간 수준인데에 비해 재가시설수가 더 많은 것을 볼 수 있다. 이를 통해 고령화율보다는 고령인구의 절대수가 시설 공급에 더 큰 영향을 줄 수 있다는 것을 알 수 있다.")
    st.write("- **인사이트 2:** 강북구/도봉구를 보면 고령화율이 1-2위인데 재가시설 수는 평균 수준 이하로 나타나고 있다. 이는 재가시설 공급이 부족한 복지 사각지대일 가능성이 존재한다는 것을 알 수 있다.")
